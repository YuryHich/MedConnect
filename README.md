...
Чат консультации в реальном времени. Пациент и врач подключаются к комнате
`consultation:<id>` через Socket.IO. JWT проверяется на handshake, история
сообщений хранится в MongoDB.

### Запуск

```bash
# терминал 1 – сервер (Socket.IO слушает тот же порт, что и REST)
cd server
npm install
...
cd client
npm install
copy .env.example .env # VITE_API_URL и VITE_SOCKET_URL
npm run dev            # http://localhost:5173
```

Войдите пациентом в одном окне браузера и врачом в другом, откройте одну и ту же
консультацию кнопкой «Открыть чат».

### События Socket.IO

| Событие | Направление | Назначение |
|---------|-------------|------------|
| `join` / `leave` | клиент → сервер | вход и выход из комнаты консультации |
| `chat:message` | оба направления | текст сообщения (acknowledgement) |
| `chat:typing` | оба направления | индикатор «печатает» |
| `chat:system` | сервер → комната | подключение и отключение участников |
| `GET /messages?consultationId=` | REST | история без сокета |

Кнопка «Видеозвонок» – заглушка будущего WebRTC.
